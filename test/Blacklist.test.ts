...
describe("Blacklist", function () {
    describe("deployment", () => {
        it("deploys as expected", async () => {
            ...
            assert(owner == admin.address, `Contract not set up correctly. Owner: ${owner} | Expected: ${admin.address}`)
        })  
    })
    ...
    describe("events", function () {
        it("AddedToBlacklist", async () => {
            ...
            await addToBlacklistWithEvents(blacklistContract, admin, player1.address, "AddedToBlacklist", [player1.address])
        })
        it("RemovedFromBlacklisted", async () => {
            ...
            await removeFromBlackListWithEvents(blacklistContract, admin, player1.address, "RemovedFromBlacklisted", [player1.address])
        })
    })
    ...
    describe("errors", function () {
        ...
        it("AccountAlreadyBlacklisted", async () => {
            ...
            await addToBlacklistWithErrors(blacklistContract, admin, player1.address, `AccountAlreadyBlacklisted`, [player1.address])
        })
        it("AccountNotBlacklisted", async () => {
            const { admin, player1 } = await getAccounts()
            const blacklistContract = await loadFixture(deployBlacklistFixture)

            await removeFromBlackListWithErrors(blacklistContract, admin, player1.address, "AccountNotBlacklisted", [player1.address])
        })
    })
});
