...
import { parseEther, solidityPacked, toUtf8String } from "ethers";
...
const callWithSyncFeeBoth = async (tx: Transaction, feeToken: string) => {
  ...
  await setBalance(gelato.address, parseEther("1"));
...
const sponsoredCallLocal = async (
  ...
  return deployer.sendTransaction({ to: request.target, data: toUtf8String(request.data) });
};
